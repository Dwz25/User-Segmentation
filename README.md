...
2. **可视化结果和营销策略**  
&emsp;&emsp;所有图片均可在文件夹 **images** 中查看，包括分布分析、相关性分析、用户分层、ROI 对比等。
3. **用户分群明细**   
...
2. **用户分层和营销策略**  
  ...
  　　结合分层雷达图（图 5）和用户分层分布图（图 6），重点分析以下高价值 / 高潜力群体：
- 核心 VIP（1.8%）：综合得分最高（90.03），R、F、M、I 四项指标均优秀，是平台最核心的价值用户，贡献了稳定的高频高消费。
- 高潜流失客（5.7%）：平均收入高达 121683 元，平均消费 4131.23 元，但最近登录时间较长（R 得分低），处于流失边缘，挽回价值极高。
- 犹豫土豪（0.6%）：平均收入全平台最高（125519 元），意向度 > 70，但购买频率和消费金额极低。结合收入与平均客单价强相关的发现，这部分用户一旦转化，单次消费金额会非常高，是转化率提升空间最大的群体。
- 高潜用户（1.2%）：高收入 + 高意向，但被传统 RFM 误判为低价值用户，是精准营销的重点对象。
